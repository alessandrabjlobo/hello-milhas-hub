      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Card>
          ...
          <CardContent>
            <div className="flex flex-wrap gap-2">
              ...
              <Button variant="outline" size="sm" onClick={() => setPresetRange("thisYear")}>
                Ano atual
              </Button>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <MetricsCard
            ...
            change={`${formatCurrency(kpis.averageCostPerThousand / 1000)}/milha`}
            icon={TrendingUp}
            trend="up"
          />
        </div>
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{formatCurrency(kpis.grossRevenue)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Faturamento com juros:</span>
                    <span className="font-semibold">{formatCurrency(kpis.revenueWithInterest)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Custo de milhas:</span>
                    <span className="font-semibold text-destructive">-{formatCurrency(kpis.totalMilesCost)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Taxas de embarque:</span>
                    <span className="font-semibold text-destructive">-{formatCurrency(kpis.totalBoardingFees)}</span>
                  </div>
                  <div className="border-t pt-2 flex justify-between">
                    <span className="font-semibold">Lucro bruto:</span>
                    <span className="font-bold text-primary">{formatCurrency(kpis.grossProfit)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Margem bruta:</span>
                    <span className="font-semibold">{kpis.grossMarginPercent.toFixed(2)}%</span>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                ...
                <CardContent className="space-y-2">
                  ...
                  <div className="flex justify-between">
                    ...
                    <span className="font-semibold">{formatCurrency(kpis.averageTicket)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Milhas totais:</span>
                    <span className="font-semibold">{formatNumber(kpis.totalMilesUsed)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Custo médio/mil:</span>
                    <span className="font-semibold">{formatCurrency(kpis.averageCostPerThousand)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Canal interno:</span>
                    <span className="font-semibold">{kpis.byChannel.internal.salesCount} vendas</span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-muted-foreground">Canal balcão:</span>
                    <span className="font-semibold">{kpis.byChannel.counter.salesCount} vendas</span>
                  </div>
                </CardContent>
              </Card>
            </div>
                  ...
                  <TableBody>
                    {kpis.byAccount.map((acc) => (
                      <TableRow key={acc.accountId}>
                        ...
                        <TableCell className="text-right">{acc.salesCount}</TableCell>
                        <TableCell className="text-right">{formatNumber(acc.milesUsed)}</TableCell>
                        <TableCell className="text-right">{formatCurrency(acc.revenue)}</TableCell>
                        <TableCell className="text-right">{formatCurrency(acc.profit)}</TableCell>
                        <TableCell className="text-right">
                          <Badge variant={acc.marginPercent > 0 ? "default" : "destructive"}>
                  ...
                  <TableBody>
                    {kpis.byAirline.map((airline) => (
                      <TableRow key={airline.airlineId}>
                        <TableCell className="font-medium">{airline.airlineName}</TableCell>
                        <TableCell className="text-right">{airline.salesCount}</TableCell>
                        <TableCell className="text-right">{formatCurrency(airline.revenue)}</TableCell>
                        <TableCell className="text-right">{formatCurrency(airline.profit)}</TableCell>
                        <TableCell className="text-right">
                          <Badge variant={airline.marginPercent > 0 ? "default" : "destructive"}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <Card>
                ...
                <CardContent className="space-y-2">
                  <div className="flex justify-between">
                    <span>Vendas:</span>
                    <span className="font-semibold">{kpis.byChannel.internal.salesCount}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Receita:</span>
                    <span className="font-semibold">{formatCurrency(kpis.byChannel.internal.revenue)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Lucro:</span>
                    <span className="font-bold text-primary">{formatCurrency(kpis.byChannel.internal.profit)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Margem:</span>
                    <Badge>{kpis.byChannel.internal.marginPercent.toFixed(1)}%</Badge>
                  </div>
                </CardContent>
              </Card>
              ...
              <Card>
                <CardHeader>
                  ...
                  <CardDescription>Vendas com fornecedores</CardDescription>
                </CardHeader>
                <CardContent className="space-y-2">
                  <div className="flex justify-between">
                    <span>Vendas:</span>
                    <span className="font-semibold">{kpis.byChannel.counter.salesCount}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Receita:</span>
                    <span className="font-semibold">{formatCurrency(kpis.byChannel.counter.revenue)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Lucro:</span>
                    <span className="font-bold text-primary">{formatCurrency(kpis.byChannel.counter.profit)}</span>
                  </div>
                  <div className="flex justify-between">
                    <span>Margem:</span>
                    <Badge>{kpis.byChannel.counter.marginPercent.toFixed(1)}%</Badge>
                  </div>
                </CardContent>
              </Card>
            </div>
                  ...
                  <TableBody>
                    {kpis.byPaymentMethod.map((pm) => (
                      <TableRow key={pm.method}>
                        ...
                        <TableCell className="text-right">{pm.salesCount}</TableCell>
                        <TableCell className="text-right">{formatCurrency(pm.revenue)}</TableCell>
                        <TableCell className="text-right">{formatCurrency(pm.averageTicket)}</TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
                </Table>
